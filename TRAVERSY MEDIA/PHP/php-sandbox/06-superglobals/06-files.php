...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
  ...
  if ($file['error'] === UPLOAD_ERR_OK) {
    ...
    // Check  file type
    $allowerExtensions = ['jpg', 'png', 'jpeg'];
    $fileExtension = strtolower(pathinfo($filename, PATHINFO_EXTENSION));

    // Upload file
    if (in_array($fileExtension, $allowerExtensions) && move_uploaded_file($file['tmp_name'], $uploadDir . $filename)) {
      echo 'File uploaded!';
    } elseif (!in_array($fileExtension, $allowerExtensions)) {
